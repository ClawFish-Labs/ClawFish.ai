<footer id="footer" class="mt-24 border-t border-white/5 bg-black">
  <div class="mx-auto max-w-7xl px-4 py-14">
    <div class="grid gap-12 lg:grid-cols-12">
      <div class="lg:col-span-4">
        <div class="flex items-center gap-3">
          <div class="logo-mark">C</div>
          <div class="leading-tight">
            <div class="text-sm font-semibold tracking-[0.24em] text-white">CLAWFISH</div>
            <div class="text-[11px] text-white/40">AI COMPANY</div>
          </div>
        </div>

        <p class="mt-5 max-w-sm text-sm leading-6 text-white/55">
          Clawfish Labs builds focused AI products with a clean and modern edge.
        </p>
      </div>

      <div class="grid gap-10 sm:grid-cols-2 lg:col-span-5 lg:grid-cols-3">
        <div>
          <h3 class="text-sm font-semibold text-white">Product</h3>
          <ul class="mt-4 space-y-3 text-sm text-white/50">
            <li><a href="#features" class="hover:text-white transition-colors">Features</a></li>
            <li><a href="#platform" class="hover:text-white transition-colors">Platform</a></li>
            <li><a href="#pricing" class="hover:text-white transition-colors">Pricing</a></li>
          </ul>
        </div>

        <div>
          <h3 class="text-sm font-semibold text-white">Company</h3>
          <ul class="mt-4 space-y-3 text-sm text-white/50">
            <li><a href="#company" class="hover:text-white transition-colors">Clawfish Labs</a></li>
            <li><a href="https://github.com/ClawFish-Labs" class="hover:text-white transition-colors">GitHub</a></li>
            <li><a href="/platform" class="hover:text-white transition-colors">Try Clawfish</a></li>
          </ul>
        </div>

        <div>
          <h3 class="text-sm font-semibold text-white">Status</h3>
          <ul class="mt-4 space-y-3 text-sm text-white/50">
            <li>Still in beta.</li>
            <li>More soon.</li>
            <li>Building now.</li>
          </ul>
        </div>
      </div>

      <div class="lg:col-span-3">
        <div class="rounded-3xl border border-white/10 bg-white/3 p-6">
          <p class="text-sm font-semibold text-white">Clawfish Labs</p>
          <a href="/platform" class="mt-5 inline-flex rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-2 text-sm font-medium text-orange-300 transition hover:bg-orange-500/20 hover:text-white">
            Try Clawfish
          </a>
        </div>
      </div>
    </div>

    <div class="mt-12 flex flex-col gap-4 border-t border-white/5 pt-6 text-sm text-white/40 sm:flex-row sm:items-center sm:justify-between">
      <p>© 2026 Clawfish Labs. All rights reserved.</p>
      <div class="flex gap-4">
        <a href="/" class="hover:text-white transition-colors">Home</a>
        <a href="#footer" class="hover:text-white transition-colors">Footer</a>
      </div>
    </div>
  </div>
</footer>